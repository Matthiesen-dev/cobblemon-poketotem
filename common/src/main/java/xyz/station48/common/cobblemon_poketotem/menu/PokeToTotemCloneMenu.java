...
public class PokeToTotemCloneMenu implements MenuProvider {
    private final Container container = new SimpleContainer(9 * 4);
    private final ServerPlayer playerToView;
    ...
    private void refresh(ServerPlayer player, PartyStore storage) {
        for (int j = 0; j <= 5; j++) {
            Pokemon pokemon = storage.get(j);
            int slotIndex = 12 + j + (j >= 3 ? 6 : 0);
